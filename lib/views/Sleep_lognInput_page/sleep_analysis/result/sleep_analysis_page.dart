...
class _SleepAnalysisResultPageContentState
  ...
  List<Map<String, dynamic>> _asListOfMaps(dynamic l) {
    // Normalize a dynamic value into a list of maps. Returns an empty list
    // if the input is null or not a list. Non-map entries are converted
    // into empty maps to preserve the original list length.
    ...
    if (l is List) {
      return l.map((e) {
        ...
        // For unsupported types, return an empty map so downstream code
        // can still iterate without null checks.
        return <String, dynamic>{};
      }).toList();
    }
  ...
  void _parseProfessionalAnalysis(
    ...
    try {
      ...
      setState(() {
        ...
        final detailedRaw =
            safeParsed['detailedReport'] ?? safeParsed['detailed_report'];
        final detailed = detailedRaw?.toString().trim();
  ...
  _fetchAndAnalyzeLastLog() async {
    try {
      ...
      // Build current + small history (most recent first)
      final docs = query.docs;
      final logsObjs = <SleepLog>[];
      for (final d in docs) {
        final map = (d.data() is Map) ? Map<String, dynamic>.from(d.data()) : <String, dynamic>{};
        logsObjs.add(SleepLog.fromMap(map, d.id));
      }
      // Sort by date desc if not already
      // (Firestore orderBy('date', descending: true) ensures desc order)
      _recentLogs = logsObjs.take(7).toList();
                        ...
                        builder: (tabContext) {
                          ...
                          return _wrapTabWithSliverWidget(
                            ...
                            ReportTab(
                              ...
                              sleepScore: _sleepScore.toDouble(),
                            ),
                          );
                        },
                      ),
