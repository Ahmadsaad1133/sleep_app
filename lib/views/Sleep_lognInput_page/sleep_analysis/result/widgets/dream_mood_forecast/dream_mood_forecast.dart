...
class _DreamLabScreenState extends State<DreamLabScreen> with AutomaticKeepAliveClientMixin {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [
          ...
          BoxShadow(
            color: Color(0xFF2575FC).withOpacity(0.08),
            blurRadius: 20.r,
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          // Header row for the card.  We retain the original icon colour but
          // soften the background to blend with the new card gradient.
          Container(
  ...
  Widget _buildTechnicalToggle() {
    return Container(
      ...
      child: ListTile(
        ...
        leading: Icon(
          _showTechnical ? Icons.code_off : Icons.code,
  ...
  Widget _buildTechnicalData() {
    ...
    final prettyJson = encoder.convert(_rawApiResponse ?? _rootMap());
    return _buildClinicalCard(
      title: _ui('card_technical_data') ?? 'Technical Data',
  ...
  Widget _buildMoodIndicator(String mood) {
    ...
    final icon = moodIcons[mood.toLowerCase()] ?? Icons.mood;
    final color = moodColors[mood.toLowerCase()] ?? Colors.white;
  ...
  Color _getMoodColor(String mood) {
    switch (mood.toLowerCase()) {
      case 'positive': return Colors.green[300]!;
      case 'neutral': return Colors.amber[300]!;
      case 'negative': return Colors.red[300]!;
  ...
  double _parseConfidence(String confidence) {
    try {
      final clean = confidence.replaceAll(RegExp(r'[^0-9.]'), '');
